...
| `--update` | Update CLAUDE.md block only -- re-scan skills/agents and refresh the block |
...
Dynamically scan the plugin directories to build the block:
...
find agents/ -name "*.md" -maxdepth 1 | sort
...
Git & Version Control:
  ...
  /air-claudecode:git-flow-master   Git Flow lifecycle (start/finish/publish/update/delete)
...
Setup:
  /air-claudecode:setup-air-cc      This setup wizard

### Keyword Triggers

Skills are auto-detected from natural language:
  "commit", "커밋"              -> git-commit
  "create branch", "브랜치"     -> git-branch
  "pr", "pull request"          -> git-pr-master
  "git flow", "깃플로우"        -> git-flow-master
  "jira", "지라"                -> jira-master
  "slack", "슬랙"               -> slack-master
  "review", "리뷰"              -> code-review
  "implement", "구현"           -> software-engineer
  "test", "테스트"              -> test-engineer
  "sql", "쿼리"                -> sql-generator
  "calendar", "일정"            -> gog-calendar

<!--air-claudecode:end-->
```

**Important**: The skill list and keyword triggers should be generated by scanning actual files, not hardcoded. The template above is an example of the output format.
...
- **Block exists**: Replace content between the markers with updated content
...
USAGE:
  ...
  /air-claudecode:setup-air-cc --update   Update CLAUDE.md block only (re-scan skills/agents)
...
WHAT IT CHECKS:
  ...
  CLAUDE.md
    - ~/.claude/CLAUDE.md           Managed block with skill catalog and keyword triggers
...
EXAMPLES:
  ...
  /air-claudecode:setup-air-cc --update    # Refresh CLAUDE.md block after adding skills
```
...
- [ ] CLAUDE.md block created/updated with dynamically scanned skill catalog
